#!/usr/bin/env python3
"""Run blind post-seed pre-eval for holdout_*.png images only.

Conservative operator helper for BRAIN_INTERVIEW_PROMPT holdout workflow.
It ignores leftover seed_*.png files, writes perception/pre_eval via validate_eval.py --append-jsonl,
and does not move, final_eval, or embed holdout files.
"""
...
DRAFT_MD = ROOT / 'benchmark/anchors/holdout.post_seed.draft.md'
DRAFT_JSONL = ROOT / 'benchmark/anchors/holdout.post_seed.draft.jsonl'
STATE_FILE = ROOT / 'benchmark/anchors/.holdout_post_seed_preeval_session_id'
TMP = ROOT / 'benchmark/tmp'
PERCEPTION_LOG = LOGS / 'perception_history.jsonl'
PREEVAL_LOG = LOGS / 'pre_eval_history.jsonl'
IMAGE_ORIGIN = 'holdout_benchmark'
CREATED_BY = 'unknown'
...
def worksheet_section(rec: dict) -> str:
    ...
    lines = [
        ...
        "",
        "Brain blind post-seed prediction:",
        f"- Promotion: `{rec['brain_initial_anchor_promotion_recommendation']}`",
        f"- Score raw: `{md_escape(rec.get('brain_initial_score_raw'))}`",
        f"- Gold similarity: `{md_escape(rec.get('brain_initial_gold_similarity'))}`",
        f"- Anti similarity: `{md_escape(rec.get('brain_initial_anti_similarity'))}`",
        f"- Rank: `{md_escape(rec.get('brain_initial_rank'))}`",
...
def main():
    ...
    # IMPORTANT: process holdout_*.png only; ignore leftover seed_*.png non-promoted anchor seeds.
    images = sorted([p for p in INBOX.glob('holdout_*.png') if p.is_file()])
    ...
    md_header = [
        '# Holdout benchmark post-seed blind pre-eval',
        '',
        ...
        '',
        'Blind prediction only. Do not add Aaron holdout labels, scores, or rationale here; external evaluator handles holdout ground truth.',
        '',
    ]
    ...
    (TMP / 'holdout_post_seed_draft_validation.stdout.jsonl').write_text(validation_out + ('\n' if validation_out else ''))
    if validation_err:
        (TMP / 'holdout_post_seed_draft_validation.stderr.txt').write_text(validation_err + '\n')
    if p.returncode != 0:
        raise RuntimeError(f"draft JSONL validation failed; stdout saved to {TMP / 'holdout_post_seed_draft_validation.stdout.jsonl'}\n{validation_out}\nSTDERR:\n{validation_err}")
    summary = {
        'ok': True,
        'session_id': session_id,
        'images': len(records),
        ...
        'validation_stdout': str(TMP / 'holdout_post_seed_draft_validation.stdout.jsonl'),
        **append_counts,
    }
